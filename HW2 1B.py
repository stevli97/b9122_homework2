...
abc = "https://www.sec.gov/news/press-release"
...
while len(urls) > 0 and len(opened) < maxNumUrl:
    ...
    for tag in soup.find_all('a', href = True): 
        ...
        if abc in childUrl and childUrl not in seen and len(y) < 20:
            ...
            txt = soup1.get_text()
            x = txt.lower().count("charges")
            tyt = soup1.find_all(class_="article-body")
            print(x)
...
print("num. of URLs seen = %d, and scanned = %d, and target = %d" % (len(seen), len(opened),len(y)))
